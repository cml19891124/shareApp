...
//初始化函数
-(BOOL) initAPPID:(NSString *)app_id mch_id:(NSString *)mch_id notifyUrl_url:(NSString *)notify_url
{
    //初始构造函数
...
- ( NSMutableDictionary *)sendPay_demo:(NSString *)name :(float)price :(NSString *)attach tradeNo:(NSString *)tradeNo
{
  ...
  //  NSString *order_name    = name;
   ...
   NSString * pricestr0 = [NSString stringWithFormat:@"%f",pricefloat];
    
    int priceint = [pricestr0 intValue];
    
   NSString * pricestr = [NSString stringWithFormat:@"%d",priceint];
    
   NSString *order_price   = pricestr;//1分钱测试
...
//   NSString *order_price   = @"2";//1分钱测试
    ...
    [packageParams setObject: @"APP" forKey:@"trade_type"];  //交易类型，固定为APP
    [packageParams setObject: orderno forKey:@"body"];        //订单描述，展示给用户
    ...
    [packageParams setObject: attach forKey:@"attach"];       //订单金额，单位为分
