...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [self.toolBarContainer.toolbar drawRect:self.toolBarContainer.toolbar.frame];
    ...
    [_conversation refreshTargetInfoFromServer:^(id resultObject, NSError *error) {
        HPLog(@"refresh nav right button");
        kSTRONGSELF
...
- (void)viewDidLayoutSubviews {
  HPLog(@"Event - viewDidLayoutSubviews");
...
- (void)addtoolbar {
  self.toolBarContainer.toolbar.frame = CGRectMake(0, 0, kScreenWidth, 45);
...
- (void)onSendMessageResponse:(JMSGMessage *)message error:(NSError *)error {
  ...
  if (error != nil) {
    HPLog(@"Send response error - %@", error);
    [_conversation clearUnreadCount];
    ...
    [MBProgressHUD hideHUDForView:self.view animated:YES];
      [HPProgressHUD alertMessage:alert];
    return;
  }
...
- (void)onReceiveMessage:(JMSGMessage *)message error:(NSError *)error {
    ...
    HPLog(@"Event - receiveMessageNotification");
    
    kWEAKSELF
    JCHATMAINTHREAD((^{
        kSTRONGSELF
        if (!message) {
          HPLog(@"get the nil message .");
          return;
        }
...
- (void)relayoutTableCellWithMessage:(JMSGMessage *) message{
    HPLog(@"relayoutTableCellWithMessage: msgid:%@",message.msgId);
...
//    [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
    ...
    [JMSGUser logout:^(id resultObject, NSError *error) {
      HPLog(@"Logout callback with - %@", error);
    }];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
  ...
  if ([mediaType isEqualToString:@"public.movie"]) {
    [self dismissViewControllerAnimated:YES completion:nil];
      [HPProgressHUD alertMessage:@"不支持视频发送"];
    return;
  }
  UIImage *image;
...
- (void)prepareImageMessage:(UIImage *)img {
  HPLog(@"Action - prepareImageMessage");
...
- (void)prepareTextMessage:(NSString *)text {
    ...
    HPLog(@"Action - prepareTextMessage");
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
  if (!isNoOtherMessage) {
    ...
    if (indexPath.row >= _allmessageIdArr.count) {
        HPLog(@"1.index %ld beyond bounds %ld",indexPath.row,_allmessageIdArr.count);
        return 40;
    }
...
//          [self performSelector:@selector(flashToLoadMessage) withObject:nil afterDelay:0];
    ...
    if (indexPath.row >= _allmessageIdArr.count) {
        HPLog(@"2.index %ld beyond bounds %ld",indexPath.row,_allmessageIdArr.count);
        return nil;
    }
    NSString *messageId = _allmessageIdArr[indexPath.row];
    if (!messageId) {
        HPLog(@"messageId is nil%@",messageId);
        return nil;
    }

    JCHATChatModel *model = _allMessageDic[messageId];
    if (!model) {
        HPLog(@"JCHATChatModel is nil%@", messageId);
        return nil;
    }
...
- (void)chcekReceiveMessageAvatarWithReceiveNewMessage:(JMSGMessage *)message {
    HPLog(@"chcekReceiveMessageAvatarWithReceiveNewMessage:%@",message.serverMessageId);
...
- (void)refreshVisibleRowsAvatarWithNewMessage:(JMSGMessage *)message avatarDataLength:(NSUInteger)length {
    ...
    HPLog(@"refreshVisibleRowsAvatarWithNewMessage::%@",message.serverMessageId);
...
- (void)reloadAllCellAvatarImage {
    HPLog(@"Action -reloadAllCellAvatarImage");
...
- (void)selectHeadView:(JCHATChatModel *)model {
  if (!model.message.fromUser) {
      [HPProgressHUD alertMessage:@"该用户为API用户"];
    return;
  }
...
- (void)didStartRecordingVoiceAction {
  HPLog(@"Action - didStartRecordingVoice");
  [self startRecord];
}

- (void)didCancelRecordingVoiceAction {
  HPLog(@"Action - didCancelRecordingVoice");
  [self cancelRecord];
}

- (void)didFinishRecordingVoiceAction {
  HPLog(@"Action - didFinishRecordingVoiceAction");
  [self finishRecorded];
}

- (void)didDragOutsideAction {
  HPLog(@"Action - didDragOutsideAction");
  [self resumeRecord];
}

- (void)didDragInsideAction {
  HPLog(@"Action - didDragInsideAction");
  [self pauseRecord];
}
...
- (void)SendMessageWithVoice:(NSString *)voicePath
  ...
  HPLog(@"Action - SendMessageWithVoice");
...
- (void)layoutAndAnimateMessageInputTextView:(UITextView *)textView {
  ...
  if (changeInHeight != 0.0f) {
    ...
    [UIView animateWithDuration:0.25f
                     animations:^{
                       [weakSelf setTableViewInsetsWithBottomValue:self.messageTableView.contentInset.bottom + changeInHeight];
