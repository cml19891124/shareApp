...
- (void)viewDidLoad {
  ...
  self.groupTab = [[JCHATChatTable alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth,self.view.frame.size.height - 64)];
  ...
  _headView = [[UIScrollView alloc]initWithFrame:CGRectMake(10, 0, kScreenWidth, kheadViewHeight)];
  self.groupTab.tableHeaderView = _headView;
  [_headView setBackgroundColor:[UIColor clearColor]];
  _headLine = [[UIView alloc]initWithFrame:CGRectMake(0, kheadViewHeight-1, kScreenWidth, 1)];
      ...
      if (i*4 +j == [_groupData count]) {
        ...
        [user thumbAvatarData:^(NSData *data, NSString *objectId, NSError *error) {
          if (error == nil) {
            ...
            HPLog(@"JCHATDetailsInfoVC thumbAvatarData fail");
            [personView.headViewBtn setImage:[UIImage imageNamed:@"headDefalt"] forState:UIControlStateNormal];
          }
        }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  if (indexPath.row !=[_groupTitleData count]-1) {
    ...
    if (cell == nil) {
      ...
      UIView *cellLine = [[UIView alloc]initWithFrame:CGRectMake(0, 53, kScreenWidth, 1)];
    ...
    if (cell == nil) {
      ...
      [deleteBtn setFrame:CGRectMake(20, 10, kScreenWidth - 20*2, 40)];
...
//        [MBProgressHUD hideAllHUDsForView:weakSelf.view animated:YES];
        if (error == nil) {
          HPLog(@"推出群组成功");
            [HPProgressHUD alertWithFinishText:@"推出群组成功"];
          ...
          HPLog(@"推出群组失败");
            [HPProgressHUD alertWithFinishText:@"推出群组失败"];
        }
    ...
    if (buttonIndex ==1) {
        [HPProgressHUD alertWithLoadingText:@"更新群组名称"];
      kWEAKSELF
...
//        [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
        ...
        if (error == nil) {
          JCHATMAINTHREAD(^{
              JCHATGroupSettingCell * cell = (JCHATGroupSettingCell *)[self->_groupTab cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
            ...
            [HPProgressHUD alertWithFinishText:@"更新群组名称失败"];
        }
      }];
    }
  }
