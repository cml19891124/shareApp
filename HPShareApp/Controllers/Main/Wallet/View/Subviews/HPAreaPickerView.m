...
- (void)setupModalView:(UIView *)view
{
    ...
    self.cityArray = [NSMutableArray array];
        ...
        for (NSDictionary *objects in self.areaArray) {
            [self.provinceArray addObjectsFromArray:objects.allKeys];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (component == 0) {
        ...
        //遍历出所有市
        for (NSArray *city in self.areaArray[index])
        {
            //将遍历出来市追加到存放市的集合里
//            if ([self.provinceArray[row] isEqualToString:city[_province]]) {
