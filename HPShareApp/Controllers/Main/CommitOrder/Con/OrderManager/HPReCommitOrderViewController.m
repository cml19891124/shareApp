...
#import "HPReCommitOrderViewController.h"
...
@interface HPReCommitOrderViewController ()
...
@implementation HPReCommitOrderViewController
...
- (HPCalenderView *)calenderView
{
    if (!_calenderView) {
        ...
        _calenderView.confirmTheDateBlock = ^(NSString *startDate, NSString *endDate, YZXTimeToChooseType selectedType) {
            ...
            weakSelf.orderListView.dayRent = weakSelf.model.order.totalFee;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [kNotificationCenter postNotificationName:lastCarlenderhasOrderArrayName object:nil userInfo:@{@"lastArray":self.hasOrderArray,@"totalFee":self.model.order.totalFee}];
...
- (void)getHasPredictOrderApi
{
    ...
    [HPHTTPSever HPGETServerWithMethod:method isNeedToken:YES paraments:@{@"spaceId":_model.order.spaceId?@([_model.order.spaceId integerValue]):@""} complete:^(id  _Nonnull responseObject) {
        if (CODE == 200) {
...
- (void)loadData
{
    ...
    self.locationLabel.text = [NSString stringWithFormat:@"拼租位置:%@",_model.spaceDetail.rentPlace?_model.spaceDetail.rentPlace:@"--"];
    
    if (_model.spaceDetail.picture.url.length == 0||!_model.spaceDetail.picture.url) {
    ...
    self.priceLabel.text = [NSString stringWithFormat:@"¥ %.02f",_model.order.totalFee.floatValue];
    [self.spaceInfoLabel setText:[NSString stringWithFormat:@"场地规格:%@",self.model.spaceDetail.areaRange]];
    
    if (_model.spaceDetail.rentOutside.integerValue == 1) {
...
-(void)setUpCommitSubviewsmasonry
{
    ...
    __block CGFloat locW = BoundWithSize(_model.spaceDetail.rentPlace?_model.spaceDetail.rentPlace:@"拼租位置：--", kScreenWidth, 12.f).size.width;
    ...
    if (!_predictView) {
        ...
        _predictView.knownBlock = ^{
            for (UIViewController *controller in weakSelf.navigationController.viewControllers) {
                if ([controller isKindOfClass:[HPOrderListViewController class]]) {
...
- (void)onClickOrderListBtn:(HPRightImageButton *)button
{
    ...
    if (button.selected) {
        ...
        if (verbDaysArray.count >= 1) {
            ...
            NSString *dayRent;
            if (self.model.order.totalFee.length!= 0) {
                dayRent = self.model.order.totalFee;
            }
...
- (void)createSpaceRentOrder
{
    ...
    dic[@"shareOrder"] = @"";
    dic[@"spaceId"] = _model.order.spaceId;
